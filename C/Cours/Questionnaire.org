...
3. Ecrivez l'instruction qui attribue l'adresse de la variable radius de type float a un pointeur.
   #+begin_src c
   float *ptr = &radius;
   #+end_src

4. Trouvez deux methodes pour attribuer la valeur 100 au troisieme element du tableau data[].
   #+begin_src c
   data[2] = 100;
   *(data + 2) = 100;
   #+end_src

5. Ecrivez la fonction somtabs() qui, recevant deux tableaux en argument, additionne la valeur des elements des deux tableaux,
